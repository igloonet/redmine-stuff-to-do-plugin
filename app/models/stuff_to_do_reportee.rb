class StuffToDoReportee < ActiveRecord::Base
  ...
  validate :validate_reportee_or_group, :validate_reportee, :validate_group
	...
	def self.user_base
		User.active.all(:include => :custom_values,
  ...
  def self.reportees_for(user)
      reportees = user.pref[:stuff_to_do_view_all_reportees] ? user_base : user_reportees_for(user) | reportees_from_groups_for(user)
      return reportees
  end
